...
%far detector
\newduneabbrev{fd}{FD}{far detector}{The \SI{70}{kt} total (\fdfiducialmass fiducial) mass \gls{lartpc} DUNE detector, composed of four \gls{lar}mass total (\nominalmodsize fiducial) mass modules,  %or more generally the experimental 
...
\newduneabbrev{tpc}{TPC}{time projection chamber}{A type of particle detector that uses an \efield together with a sensitive volume of gas or liquid, e.g., \gls{lar}, to perform a \threed reconstruction of a particle trajectory or interaction. The activity is recorded by digitizing the waveforms of current
  ...
  or is collected on the electrode} % The portion of each DUNE \gls{detmodule} that records ionization electrons after they drift away from a cathode through the \gls{lar}, and also  through gaseous argon in a \gls{dp} module, is a TPC.  }
...
\newduneword{wire board}{wire board}{At the head end of the APA in the \gls{sp} TPC, stacks of electronics boards referred to as ``wire boards'' are arrayed to anchor the wires.  They also provide the connection between the wires and the cold electronics} %?? long for a word. ??
...
\newduneabbrev{larsoft}{LArSoft}{Liquid Argon Software}{A shared base of physics software across \gls{lar}tpc experiments}
% these should be abbrevs
...
\newduneword{microboone}{MicroBooNE}{The \gls{lar}tpc-based \microboone neutrino oscillation experiment at  \gls{fnal}}
...
\newduneword{wirecell}{Wire-Cell}{A tomographic automated \threed neutrino event reconstruction method for \gls{lar}tpc{}s}
\newduneabbrev{wct}{WCT}{Wire-Cell Toolkit}{A software toolkit with data flow processing components for \gls{lar}tpc noise and signal simulation, noise filtering, signal processing, and tomographic \threed ionization activity imaging}
...
\newduneabbrev{citf}{CITF}{cryogenic instrumentation test facility}{A facility at \fnal with small ($<\,\SI{1}{ton}$) to intermediate ($\sim\,\SI{1}{ton}$) volumes of instrumented, purified TPC-grade \gls{lar}, used for testing devices intended for use in \gls{dune}}
